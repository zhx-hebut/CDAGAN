...
class SSAGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_M', type=float, default=1.0, help='weight for SSA loss (L1 loss between maskA to attA)')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'ssa_A']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        visual_names_A = ['real_A', 'att_A_viz', 'fake_B', 'masked_fake_B', 'rec_A']
        visual_names_B = ['real_B', 'att_B_viz', 'fake_A', 'masked_fake_A', 'rec_B']
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'G_att_A', 'G_att_B']
        else:  # during test time, only load Gs
            self.model_names = ['G_A', 'G_B', 'G_att_A', 'G_att_B']

        # define networks (both Generators and discriminators)
        # The naming is different from those used in the paper.
        # Code (vs. paper): G_A (G), G_B (F), D_A (D_Y), D_B (D_X)
        self.netG_att_A = uag.define_att_G(opt.input_nc,
                                       opt.ngf,
                                       norm=opt.norm,
        ...
        self.netG_att_B = uag.define_att_G(opt.input_nc,
                                       opt.ngf,
                                       norm=opt.norm,
                                       init_type=opt.init_type,
                                       init_gain=opt.init_gain,
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids,use_se=opt.use_se, reduction=opt.reduction)
        ...
        if self.isTrain:  # define discriminators
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids, opt.specnorm)
        ...
        if self.isTrain:
            ...
            self.masked_fake_A_pool = ImagePool(opt.pool_size)  # create image buffer to store previously generated images
            ...
            self.criterionSSA = torch.nn.L1Loss()
    ...
    def set_input(self, input):
        ...
        if not self.isTrain:
             testAtoB = self.opt.test_direction == 'AtoB'
             self.image_paths = input['A_paths' if testAtoB else 'B_paths']
    ...
    def forward(self):
        ...
        self.rec_A = self.cycle_fake_A*self.cycle_att_B + self.masked_fake_B*(1-self.cycle_att_B)

        #Cycle B
        self.cycle_att_A = self.netG_att_A(self.masked_fake_A)
        self.cycle_fake_B = self.netG_A(self.masked_fake_A)
        self.rec_B = self.cycle_fake_B*self.cycle_att_A + self.masked_fake_A*(1-self.cycle_att_A)
    ...
    def backward_G(self):
        ...
        # SSA loss
        self.loss_ssa_A = self.criterionSSA(self.att_A, self.real_M) * lambda_M
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_ssa_A
    ...
    def optimize_parameters(self,epoch):
        ...
        if epoch < 60:#and self.opt.use_early_stopping:
            nets += [self.netG_att_A,self.netG_att_B]
