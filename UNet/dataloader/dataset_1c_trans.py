...
class BaseDataSets(Dataset):
    def __init__(self, data_dir=None, data_path_name={}, mode='train', list_vol='',list_slice='',images_ratio=1):
        ...
        if images_ratio !=1 and self.mode == "train":
            images_num = int(len(self.sample_list) * images_ratio)
    ...
    def __getitem__(self, idx):
        ...
        # 1 modal 
        imgm_path = os.path.join(self.img_dir,'t1ce','{}.png'.format(case))
        img_init = Image.open(imgm_path).convert('L')
        ...
        img_modal_np = np.expand_dims(img_init_np, axis=0)
        ...
        # print(np.unique(mask_np_3c))
        # if len(img_np.shape) == 2:
        #     img_np = np.expand_dims(img_np, axis=0)
        # if len(mask_np_3c.shape) == 2:
        #     mask_np_3c = np.expand_dims(mask_np_3c, axis=0)
